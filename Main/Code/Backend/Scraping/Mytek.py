...
def get_specs_pc_portablegamer(text):
    ...
    #Extract RAM
    ram_size = re.search(r'(?:mémoire|ram)\s*(\d+)\s*gb', text, re.IGNORECASE).group(1) if re.search(r'(?:mémoire|ram)\s*(\d+)\s*gb', text, re.IGNORECASE) else "Unknown"  # Extract RAM size
    ...
    print("Screen Match: " + screen_match)
    ...
    screen_size = re.search(r'(\d+(\.\d+)?)\s*', screen_match, re.IGNORECASE).group(0) if re.search(r'(\d+(\.\d)?)\s*', screen_match, re.IGNORECASE) else "Unknown"
    ...
    while True:
        ...
        try:
            ...
            for product in products:
                ...
                Description = product_soup.find("div", class_="value", itemprop = "description").text.strip()
